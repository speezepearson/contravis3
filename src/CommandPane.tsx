...
import { InstructionSchema, DanceSchema, RelativeDirectionSchema, RelationshipSchema, DropHandsTargetSchema, HandSchema, TakeHandSchema, ActionTypeSchema, AtomicInstructionSchema, InitFormationSchema, InstructionIdSchema, RoleSchema, RollawaySchema, splitLists, splitWithLists, instructionDuration } from './types';
import type { Instruction, AtomicInstruction, Relationship, RelativeDirection, SplitBy, DropHandsTarget, ActionType, InitFormation, TakeHand, InstructionId, Role, Rollaway, Dance } from './types';
...
const ACTION_LABELS: Record<string, string> = {
  ...
  mad_robin: 'mad robin', long_lines: 'long lines',
...
function summarizeAtomic(instr: AtomicInstruction): string {
  switch (instr.type) {
    ...
    case "long_lines": {
      const rollLabel = instr.rollaway
        ? ` (${instr.rollaway.replace('_', ' ')})`
        : '';
      return `long lines${rollLabel} (${instr.beats}b)`;
    }
  }
}
...
const ROLLAWAY_OPTIONS: (Rollaway | 'none')[] = ['none', 'lark_rtl', 'lark_ltr', 'robin_rtl', 'robin_ltr'];
const ROLLAWAY_LABELS: Record<string, string> = {
  none: 'none', lark_rtl: 'lark rtl', lark_ltr: 'lark ltr', robin_rtl: 'robin rtl', robin_ltr: 'robin ltr',
};
...
function LongLinesFields({ id, isEditing, initial, onSave, onCancel, onPreview }: SubFormProps & { initial?: Extract<AtomicInstruction, { type: 'long_lines' }> }) {
  const [rollaway, setRollaway] = useState<Rollaway | 'none'>(initial?.rollaway ?? 'none');

  useInstructionPreview(onPreview, () => ({
    id, type: 'long_lines', beats: 8,
    ...(rollaway !== 'none' ? { rollaway } : {}),
  }), [id, rollaway]);

  function save() {
    onSave(InstructionSchema.parse({
      id, type: 'long_lines', beats: 8,
      ...(rollaway !== 'none' ? { rollaway } : {}),
    }));
  }
  ...
  return (<>
    <label>
      Rollaway
      <SearchableDropdown
        options={ROLLAWAY_OPTIONS}
        value={rollaway}
        onChange={v => setRollaway(v === 'none' ? 'none' : RollawaySchema.parse(v))}
        getLabel={v => ROLLAWAY_LABELS[v] ?? v}
      />
    </label>
    <SaveCancelButtons isEditing={isEditing} onSave={save} onCancel={onCancel} />
  </>);
