...
export function DropHandsFields({ instruction, onChange, onInvalid }: SubFormProps & { instruction: Extract<AtomicInstruction, { type: 'drop_hands' }> }) {
  ...
  return (<>
    <SearchableDropdown options={DROP_TARGET_OPTIONS} value={dropTarget} onChange={v => { const t = DropHandsTargetSchema.parse(v); setDropTarget(t); tryCommit({ target: t }); }} getLabel={v => DROP_TARGET_LABELS[v] ?? v} />
  </>);
}
