...
export function generateShoulderRound(prev: Keyframe, final: FinalKeyframe, instr: Extract<AtomicInstruction, { type: 'shoulder_round' }>, scope: Set<ProtoDancerId>): KeyframeFn {
  ...
  // Precompute revolve parameters for each pair (they're t-independent)
  const revolveParams = pairs.map(({ lark, robin, dist }) => {
    const larkState = prev.dancers[lark];
    const robinState = prev.dancers[robin];
    ...
    return { lark, robin, dist, revolveCenter, larkRevolveDelta, totalAngle, revolveSign };
  });
  ...
  return (t: number) => {
    ...
    for (const rp of revolveParams) {
      const { lark, robin, dist, revolveCenter, larkRevolveDelta, totalAngle, revolveSign } = rp;
      const larkState = prev.dancers[lark];
      const robinState = prev.dancers[robin];
