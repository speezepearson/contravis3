...
import { headingAngle, headingVector, makeFinalKeyframe } from '../../types';
...
export function finalTurnAlone(prev: Keyframe, instr: Extract<AtomicInstruction, { type: 'turn_alone' }>, scope: Set<ProtoDancerId>): FinalKeyframe {
  ...
  for (const id of scope) {
    const rad = headingAngle(dancers[id].facing);
    dancers[id].facing = headingVector(rad + Math.PI);
  }
...
export function generateTurnAlone(prev: Keyframe, _final: FinalKeyframe, instr: Extract<AtomicInstruction, { type: 'turn_alone' }>, scope: Set<ProtoDancerId>): Keyframe[] {
  ...
  for (let i = 1; i < nFrames; i++) {
    ...
    const dancers = copyDancers(prev.dancers);
    for (const id of scope) {
      const rad = headingAngle(prev.dancers[id].facing);
      // Larks turn CW (+), robins turn CCW (-)
      const sign = isLark(id) ? 1 : -1;
      dancers[id].facing = headingVector(rad + sign * Math.PI * t);
    }
