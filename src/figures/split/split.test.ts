...
describe('split', () => {
  it('split by role: larks and robins do different things', () => {
    const instructions = instr([{
      ...
      larks: [{ id: tid(10), beats: 4, type: 'step', direction: { kind: 'direction', value: 'up' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } }],
      robins: [{ id: tid(11), beats: 4, type: 'step', direction: { kind: 'direction', value: 'down' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } }],
    }]);
  ...
  it('split by position: ups and downs do different things', () => {
    const instructions = instr([{
      ...
      ups: [{ id: tid(10), beats: 0, type: 'step', direction: { kind: 'direction', value: 'forward' }, distance: 0, facing: { dir: { kind: 'direction', value: 'down' }, offsetRad: 0 } }],
      downs: [{ id: tid(11), beats: 0, type: 'step', direction: { kind: 'direction', value: 'forward' }, distance: 0, facing: { dir: { kind: 'direction', value: 'up' }, offsetRad: 0 } }],
    }]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
    const last = kfs[kfs.length - 1];
  ...
  it('empty larks list: larks hold still', () => {
    const instructions = instr([{
      id: tid(1), type: 'split', by: 'role',
      larks: [],
      robins: [{ id: tid(11), beats: 4, type: 'step', direction: { kind: 'direction', value: 'up' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } }],
    }]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
    const init = initialKeyframe();
    const last = kfs[kfs.length - 1];
  ...
  it('empty downs list: downs hold still', () => {
    const instructions = instr([{
      id: tid(1), type: 'split', by: 'position',
      ups: [{ id: tid(10), beats: 4, type: 'step', direction: { kind: 'direction', value: 'across' }, distance: 0.5, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } }],
      downs: [],
    }]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
    const init = initialKeyframe();
  ...
  it('split beats are computed from sub-lists', () => {
    const instructions = instr([{
      id: tid(1), type: 'split', by: 'role',
      larks: [
        { id: tid(10), beats: 4, type: 'step', direction: { kind: 'direction', value: 'up' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } },
        { id: tid(11), beats: 4, type: 'step', direction: { kind: 'direction', value: 'down' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } },
      ],
      robins: [
        { id: tid(12), beats: 8, type: 'step', direction: { kind: 'direction', value: 'across' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } },
      ],
    }]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
  ...
  it('instructions after split continue from merged state', () => {
    const instructions = instr([
      ...
      { id: tid(2), beats: 4, type: 'step', direction: { kind: 'direction', value: 'across' }, distance: 0.5, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } },
    ]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
    const init = initialKeyframe();
    const last = kfs[kfs.length - 1];
  ...
  it('split with allemande in one list and step in the other', () => {
    const instructions = instr([{
      ...
      downs: [{ id: tid(11), beats: 8, type: 'step', direction: { kind: 'direction', value: 'up' }, distance: 1, facing: { dir: { kind: 'direction', value: 'forward' }, offsetRad: 0 } }],
    }]);
    const { keyframes: kfs } = generateAllKeyframes(instructions);
    const init = initialKeyframe();
    const last = kfs[kfs.length - 1];
