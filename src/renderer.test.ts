...
function makeKeyframe(beat: number, positions: {
  up_lark: [number, number, number];
  up_robin: [number, number, number];
  down_lark: [number, number, number];
  down_robin: [number, number, number];
}): Keyframe {
  return {
    beat,
    dancers: {
      up_lark:    { x: positions.up_lark[0],    y: positions.up_lark[1],    facing: positions.up_lark[2] },
      up_robin:   { x: positions.up_robin[0],   y: positions.up_robin[1],   facing: positions.up_robin[2] },
...
function uniformKeyframes(): Keyframe[] {
  ...
  return [
    makeKeyframe(0, { up_lark: pos(0), up_robin: pos(0), down_lark: pos(0), down_robin: pos(0) }),
    makeKeyframe(4, { up_lark: pos(1), up_robin: pos(1), down_lark: pos(1), down_robin: pos(1) }),
    makeKeyframe(8, { up_lark: pos(2), up_robin: pos(2), down_lark: pos(2), down_robin: pos(2) }),
    makeKeyframe(12, { up_lark: pos(3), up_robin: pos(3), down_lark: pos(3), down_robin: pos(3) }),
  ];
}
...
describe('getFrameAtBeat', () => {
  ...
  describe('smoothness=0 (linear)', () => {
    it('produces linear interpolation at midpoint', () => {
      ...
      expect(frame.dancers.up_lark.x).toBeCloseTo(1.5);
      expect(frame.dancers.up_lark.y).toBeCloseTo(1.5);
    });

    it('returns exact keyframe values at keyframe beats', () => {
      const kfs = uniformKeyframes();
      const frame = getFrameAtBeat(kfs, 4, 0)!;
      expect(frame.dancers.up_lark.x).toBeCloseTo(1);
    });
  });
  ...
  describe('moving-average smoothing', () => {
    ...
    function stepKeyframes(): Keyframe[] {
      ...
      return [
        makeKeyframe(0, { up_lark: pos(0), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(1, { up_lark: pos(0), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(2, { up_lark: pos(0), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(3, { up_lark: pos(0), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(4, { up_lark: pos(1), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(5, { up_lark: pos(1), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(6, { up_lark: pos(1), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(7, { up_lark: pos(1), up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(8, { up_lark: pos(1), up_robin: z, down_lark: z, down_robin: z }),
      ];
    }
    ...
    it('with smoothness=0, step function has a 1-beat ramp', () => {
      ...
      expect(getFrameAtBeat(kfs, 2.5, 0)!.dancers.up_lark.x).toBeCloseTo(0);
      // Well after the ramp: fully 1
      expect(getFrameAtBeat(kfs, 5.5, 0)!.dancers.up_lark.x).toBeCloseTo(1);
      // Mid-ramp: exactly 0.5
      expect(getFrameAtBeat(kfs, 3.5, 0)!.dancers.up_lark.x).toBeCloseTo(0.5);
    });
    ...
    it('smoothing blurs a sharp step into a gradual ramp', () => {
      ...
      const x = frame.dancers.up_lark.x;
    ...
    it('smoothing has no effect on a constant signal', () => {
      ...
      expect(frame.dancers.up_lark.x).toBeCloseTo(0);
    });
    ...
    it('smoothing has no effect on a linear signal', () => {
      ...
      const raw = getFrameAtBeat(kfs, 6, 0)!.dancers.up_lark.x;
      const smoothed = getFrameAtBeat(kfs, 6, 2)!.dancers.up_lark.x;
    ...
    it('larger window produces more smoothing', () => {
      ...
      const small = getFrameAtBeat(kfs, 2.5, 1)!.dancers.up_lark.x;
      const large = getFrameAtBeat(kfs, 2.5, 4)!.dancers.up_lark.x;
    ...
    it('smoothing handles facing angles near 0/360 boundary', () => {
      ...
      const z: [number, number, number] = [0, 0, 0];
      const kfs = [
        makeKeyframe(0, { up_lark: [0, 0, 350], up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(2, { up_lark: [0, 0, 350], up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(4, { up_lark: [0, 0, 10],  up_robin: z, down_lark: z, down_robin: z }),
        makeKeyframe(6, { up_lark: [0, 0, 10],  up_robin: z, down_lark: z, down_robin: z }),
      ];
      const frame = getFrameAtBeat(kfs, 3, 2)!;
      const f = frame.dancers.up_lark.facing;
