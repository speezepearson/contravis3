...
import { parseDancerId, InstructionSchema, DanceSchema, ProtoDancerIdSchema, NORTH, EAST, SOUTH, WEST } from './types';
import { z } from 'zod';
...
describe('generateAllKeyframes', () => {
  ...
  describe('do_si_do', () => {
    ...
    it('orbit is elliptical: perpendicular displacement is 0.25m, not full radius', () => {
      ...
      expect(mid.dancers['up_lark_0'].x).toBeCloseTo(-0.75, 1); // 0.25m west of center
      expect(mid.dancers['up_lark_0'].y).toBeCloseTo(0, 1);
      // down_robin (north of center) moves east first → at quarter turn, east side
      expect(mid.dancers['down_robin_0'].x).toBeCloseTo(-0.25, 1); // 0.25m east of center
  ...
  describe('pull_by', () => {
    ...
    it('right hand pull-by follows a CW elliptical path', () => {
      ...
      const startX = init.dancers['up_lark_0'].x; // -0.5
      expect(mid.dancers['up_lark_0'].x).toBeLessThan(startX - 0.1);
      // down_robin_0 travels south CW → curves east (x increases)
      expect(mid.dancers['down_robin_0'].x).toBeGreaterThan(startX + 0.1);
    });
    ...
    it('left hand pull-by follows a CCW elliptical path', () => {
      ...
      expect(mid.dancers['up_lark_0'].x).toBeGreaterThan(startX + 0.1);
      // down_robin_0 travels south CCW → curves west (x decreases)
      expect(mid.dancers['down_robin_0'].x).toBeLessThan(startX - 0.1);
    });
  });
