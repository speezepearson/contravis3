...
import type { BaseRelationship, ProtoDancerId } from './types';
    ...
    for (const offset of offsets) {
      for (const dancer of PROTO_DANCER_IDS) {
        it(`${base}(offset=${offset}) is symmetric for ${dancer}`, () => {
          ...
          const { proto: targetProto } = parseDancerId(target);
          const reverse = resolveRelationship({ base, offset }, targetProto);
