...
const { keyframes: kfs, errors: generationErrors } = generateAllKeyframes(
  dance.instructions,
);
...
for (const id of protoIds) {
  for (const side of ["left", "right"] as ("left" | "right")[]) {
    ...
    const key =
      aKey < targetId
        ? `${aKey}:${side}:${targetId}:${targetSide}`
        : `${targetId}:${targetSide}:${aKey}:${side}`;
